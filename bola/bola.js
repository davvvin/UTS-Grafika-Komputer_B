...
var powerUp = null;
var bola = { x: cnv.width / 2, y: cnv.height / 2, radius: 10, dx: 8, dy: -8, color: { r: 0, g: 0, b: 255 } };
...
function spawnPowerUp() {
    var warna = Math.floor(Math.random() * 2); // 0: skillPanjanginPaddle, 1: percepat bola
    var bentukSkill = Math.random() < 0.5 ? "conf" : "bola"; // 50% chance masing-masing skill
...
// function Skill_PanjanginPaddle() { // logika skill ini masih dipikirkan
//     kotakTinggi += 60;
//     setTimeout(() => {
//         kotakTinggi -= 60;
//     }, 5000);
...
function Skill_PercepatKecepatanBola() { // mempercepat Kecepatan bola
    bola.dx *= 1.5;
    bola.dy *= 1.5;
    setTimeout(() => {
        bola.dx /= 1.5;
...
function Skill_confusionKecepatanBola() {
    bola.dx /= 1.5;
    bola.dy /= 1.5;
    setTimeout(() => {
        bola.dx *= 3;
...
function polar_circle2() {
    ...
    if (powerUp && powerUp.active) {
        console.log("Power-up muncul di:", powerUp.x, powerUp.y, powerUp.skill); // testing

        // menggambar power up
        if (powerUp.warna === 0) { // kotak merah
            for (var i = -10; i <= 10; i++) {
                dda_line(imageData, powerUp.x - 10, powerUp.y + i, powerUp.x + 10, powerUp.y + i, 255, 0, 0);
            }
        } else { // kotak hijau
            for (var i = -10; i <= 10; i++) {
                dda_line(imageData, powerUp.x - 10, powerUp.y + i, powerUp.x + 10, powerUp.y + i, 0, 255, 0);
            }
        }

        // menangkap power up
        var dx = bola.x - powerUp.x;
        var dy = bola.y - powerUp.y;
        var jarak = Math.sqrt(dx * dx + dy * dy);

        if (jarak < bola.radius + 10) {
            if (powerUp.skill === "conf") {
                Skill_confusionKecepatanBola();
            } else if (powerUp.skill === "bola") {
                Skill_PercepatKecepatanBola();
            }
...
function SpawnPowerUpLooping() {
    if (!powerUp || !powerUp.active) { // spawn powerUp jika tidak ada atau sudah diambil
        spawnPowerUp();
    }
    var spawnInterval = Math.floor(Math.random() * 10000) + 5000; // 5-10 detik
    setTimeout(SpawnPowerUpLooping, spawnInterval);
}
SpawnPowerUpLooping();